...
namespace MLToolkit.Forms.SwipeCardView
{
    public class SwipeCardView : ContentView, IDisposable
    {
        ...
        private const float DefaultCardRotation = 0;
        ...
        public async Task InvokeSwipe(SwipeCardDirection swipeCardDirection, uint numberOfTouches, uint touchDifference, TimeSpan touchDelay, TimeSpan endDelay)
        {
            ...
            {
                if (swipeCardDirection == SwipeCardDirection.Down)
                {
                    differenceX = 0;
                }
        ...
        private void OnPanUpdated(object sender, PanUpdatedEventArgs e)
        {
            ...
            switch (e.StatusType)
            {
                case GestureStatus.Started:
                    float positionSExs = (float)e.TotalX;
                    float positionSEys = (float)e.TotalY;
                    HandleTouchStart();
                    break;

                case GestureStatus.Running:
                    float positionExs = (float)e.TotalX;
                    float positionEys = (float)e.TotalY;
                    SwipeCardDirection directiony = positionExs > 0 ? SwipeCardDirection.Right : positionExs < 0 ? SwipeCardDirection.Left:SwipeCardDirection.Up;

                    if (Math.Abs(positionEys) > Math.Abs(positionExs))
                    {
                        directiony = SwipeCardDirection.Down;
                    }


                    if ((directiony == SwipeCardDirection.Left || directiony == SwipeCardDirection.Right) && (positionEys < 0 || positionEys > 0))
                    {
                        positionEys = 0;
                        HandleTouch(positionExs, positionEys);
                    }
                    else
                    {
                        SwipeCardDirection direction = positionEys > 0 ? SwipeCardDirection.Down : SwipeCardDirection.Up;

                        if (direction == SwipeCardDirection.Down && (positionExs < 0 || positionExs >= 0))
                        {
                            positionExs = 0;
                            HandleTouch(positionExs, positionEys);
                        }
        ...
        private async void HandleTouchEnd()
        {
            ...
            if (position == DraggingCardPosition.FinishedOverThreshold && SupportedSwipeDirections.IsSupported(direction))
            {
                ...
                if (SwipeCardView.CurrentIdString!=null)
                {
                    SaveSwipeOptionToDatabase(direction, SwipeCardView.CurrentIdString);
                }
        ...
        private void SaveSwipeOptionToDatabase(SwipeCardDirection direction, string currentIdString)
        {
            UsersMock usersMock = new UsersMock();
            usersMock.InsertSwipeOption(direction, currentIdString);
        }
