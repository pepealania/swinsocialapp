...
{
    public partial class MyProfileUploadAvatarCrop : ContentPage
    {
        private const int BaseBlockSize = 12;
        static string ftpHost = "sub.expatcallers.com";
        static String ftpurl = $"ftp://sub.expatcallers.com/swingsocial/myprofile/avatar/"; // e.g. ftp://serverip/foldername/foldername
        static string folderUrl = $"http://sub.expatcallers.com/swingsocial/myprofile/avatar/";
        ...
        public MyProfileUploadAvatarCrop(SKImage image,string imageName,Stream stream)
        {
            globalSKImage = image;
        ...
        private async void UploadFileToFTP(FileStream fs)
        {
            try
            {
                ...
                imageUrl = "http://sub.expatcallers.com/swingsocial/myprofile/avatar/" + System.IO.Path.GetFileName(fs.Name);
        ...
        public static async Task<bool> UploadMemoryStreamToFtpAsync(MemoryStream memoryStream, string ftpHost, string ftpUser, string ftpPassword, string remoteFileName)
        {
            try
            {
                ...
                using (var client = new FtpClient(ftpHost, ftpUser, ftpPassword))
                {
                    ...
                    await Task.Run(() => client.UploadStream(memoryStream, "/swingsocial/myprofile/avatar/" + remoteFileName, FtpRemoteExists.Overwrite));
