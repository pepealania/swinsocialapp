...
{
    public partial class EventCreatePhotosPage : ContentPage
    {
        ...
        public EventCreatePhotosPage(Event ev)
        {
            currentEvent = ev;
            InitializeComponent();
        ...
        private async void Saved_Tapped(object sender, EventArgs e)
        {
            ...
            if (coverImageUrl==null || coverImageUrl==string.Empty)
            {
                coverImageUrl = "";
            }
            currentEvent.ImagesString = "''" + coverImageUrl + "''";
            EventsService epvm = new EventsService();
            var eventId = await epvm.EventInsert(currentEvent);
