...
{
    public partial class RegistrationUploadCrop : ContentPage
    {
        private const int BaseBlockSize = 12;
        static string ftpHost = "sub.expatcallers.com";
        static String ftpurl = $"ftp://sub.expatcallers.com/swingsocial/registration/"; // e.g. ftp://serverip/foldername/foldername
        static string folderUrl = $"http://sub.expatcallers.com/swingsocial/registration/";
        ...
        public static bool commingFromRegistrationCrop;
        public static SKImageImageSource currentStream;
        public static string imagePrivateUrl;
        public RegistrationUploadCrop(int order,SKImage image,string imageName,Stream stream)
        {
            globalSKImage = image;
        ...
        private async void DoUploads()
        {
            try
            {
                ...
                commingFromRegistrationCrop = true;
                var nextPage = new BRegistrationPage71();
                await Navigation.PushAsync(nextPage);
            }
            catch (Exception ex)
            {
        ...
        private async void UploadFileToFTP(FileStream fs)
        {
            try
            {
                ...
                imageUrl = "http://sub.expatcallers.com/swingsocial/registration/" + System.IO.Path.GetFileName(fs.Name);
        ...
        public static async Task<bool> UploadMemoryStreamToFtpAsync(MemoryStream memoryStream, string ftpHost, string ftpUser, string ftpPassword, string remoteFileName)
        {
            try
            {
                ...
                using (var client = new FtpClient(ftpHost, ftpUser, ftpPassword))
                {
                    ...
                    await Task.Run(() => client.UploadStream(memoryStream, "/swingsocial/registration/" + remoteFileName, FtpRemoteExists.Overwrite));
