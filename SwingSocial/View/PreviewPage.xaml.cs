...
namespace SwingSocial.Sample.View
{
    public partial class PreviewPage : ContentPage
    {
        ...
        static String ftpurl = $"ftp://sub.expatcallers.com/swingsocial/wha1"; // e.g. ftp://serverip/foldername/foldername
        static string folderUrl = $"http://sub.expatcallers.com/swingsocial/wha1/";
        ...
        public PreviewPage(SKImage image,string imageName,Stream stream)
        {
            globalSKImage = image;
        ...
        private async void Publish_Tapped(object sender, System.EventArgs e)
        {
            ...
            captionEntry.Text = "File Successfully Uploaded";
        ...
        private async void UploadFileToFTP(FileStream fs)
        {
            try
            {
                ...
                imageUrl = "http://www.expatcallers.com/swingsocial/wha1/" + System.IO.Path.GetFileName(fs.Name);
        ...
        public static async Task<bool> UploadMemoryStreamToFtpAsync(MemoryStream memoryStream, string ftpHost, string ftpUser, string ftpPassword, string remoteFileName)
        {
            try
            {
                ...
                using (var client = new FtpClient(ftpHost, ftpUser, ftpPassword))
                {
                    ...
                    await Task.Run(() => client.UploadStream(memoryStream, "/swingsocial/wha1/" + remoteFileName, FtpRemoteExists.Overwrite));
