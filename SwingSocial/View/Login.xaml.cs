...
namespace SwingSocial.Sample.View
{
    public partial class Login : ContentPage
    {
        ...
        public static bool CommingFromLogin;
        public static Page LoginPage;
        public static bool NetWorkErrorHappen;
        public Login()
        {
                    ...
                    if (fileResult)
                    {
                        ...
                        if (cookieValue != String.Empty && cookieValue != "0")
                        {
                            ...
                            else
                            {
                                ...
                                UserNameToValidate.Text = "chrisnleslie123";
        ...
        {
            bool value = await UserNameExists(UserNameToValidate.Text.Trim());
            if (value)
            {
                ...
                CommingFromLogin = true;
                var nextPage = new LoginVerifyCode(MyProfile.Email);
                await Navigation.PushAsync(nextPage);
            }
            else
            {

                if (!NetWorkErrorHappen)
                {
                    await DisplayAlert("Sign in Status", "Write a valid Username or register a new account.", "ok");

                }
                else
                {
                    NetWorkErrorHappen = false;
                }
            }
        }
        //
        ...
        {
            base.OnAppearing();
            if (!EventCreate.TransactionAlive && !WhatshotUploadFilePage.TransactionAlive && BRegistrationPage1.TransactionAlive)
            {
        ...
        private async Task<bool> UserNameExists(string text)
        {
            ...
            {
                this.DisplayAlert("Network Error", "Network Error, email technical support.", "ok");
                NetWorkErrorHappen = true;
                return false;
            }
        }
