...
{
    internal class ProfilesPageViewModel : BasePageViewModel
    {
        ...
        public ProfilesPageViewModel(RSVP rsvp,INavigation navigation)
        {
            Navigation = navigation;

            NavigateCommand = new Command<Type>(OnNavigateCommand);
            InitializeProfiles(rsvp);
            //InitializeEvents();
        ...
        public ObservableCollection<ProfileEntity> ProfilesRSVP
        {
            get => _profilesRSVP;
            set
            {
        ...
        private async void InitializeProfiles(RSVP rsvp)
        {
            UsersMock mock = new UsersMock();
            List<ProfileEntity> profiles = await mock.LoadUserProfiles(rsvp);

            foreach (var item in profiles)
            {
                ProfilesRSVP.Add(item);
            }
