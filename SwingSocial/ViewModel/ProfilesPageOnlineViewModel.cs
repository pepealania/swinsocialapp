...
{
    internal class ProfilesPageOnlineViewModel : BasePageViewModel
    {
        private ObservableCollection<Event> _events = new ObservableCollection<Event>();
        private ObservableCollection<ProfileEntity> _profilesonline = new ObservableCollection<ProfileEntity>();

        private uint _threshold;

        public ProfilesPageOnlineViewModel(INavigation navigation,ProfileEntity p)
        {
        ...
        public ObservableCollection<ProfileEntity> ProfilesOnline
        {
            get => _profilesonline;
            set
            {
                _profilesonline = value;
                RaisePropertyChanged();
            }
        }
        ...
        private async void InitializeProfiles(ProfileEntity p)
        {
            ...
            List<ProfileEntity> profiles = await mock.LoadOnlineProfiles(p);

            foreach (var item in profiles)
            {
                ProfilesOnline.Add(item);
            }
        }
